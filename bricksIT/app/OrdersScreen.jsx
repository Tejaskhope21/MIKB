    ...
    try {
      ...
      const transformed = rawOrders.map((order) => ({
        ...
        items: (order.items || []).map((item) => ({
          ...
          amount:
            Number(item?.quantity || 1) * Number(item?.price || item?.product?.price || 0),
        })),
      }));
...
====================================
    ...
    Alert.alert(
      'Invoice Generated',
      'Invoice preview (text format):\n\n' + invoiceDetails.substring(0, 500) + (invoiceDetails.length > 500 ? '...' : ''),
      [
  ...
  const renderOrderCard = ({ item, index }) => {
    ...
    return (
      ...
      >
        ...
        <TouchableOpacity style={styles.viewDetailsButton} onPress={() => handleViewOrder(item)}>
          ...
          {['all', 'pending', 'paid', 'shipped', 'delivered', 'cancelled'].map((s) => {
            ...
            return (
              <TouchableOpacity
                ...
                style={[
                  styles.filterChip,
                  active && { backgroundColor: conf.text, borderColor: conf.text },
                ]}
            ...
            <View style={styles.modalContainer}>
              ...
              <ScrollView style={{ flex: 1 }}>
                ...
                <View style={styles.section}>
                  <Text style={styles.sectionTitle}>Order #{selectedOrder.orderId || 'N/A'}</Text>
                  <Text style={styles.dateText}>Placed on {formatDate(selectedOrder.orderDate)}</Text>
                </View>
                    ...
                    <View key={i} style={styles.itemRow}>
                      <View style={{ flex: 1 }}>
                        ...
                        <Text style={styles.itemQty}>
                          {item.quantity || 0} × ₹{Number(item.price || 0).toLocaleString('en-IN')}
                        </Text>
                      </View>
                ...
                <View style={styles.section}>
                  ...
                  <Text style={styles.address}>{selectedOrder.deliveryAddress || 'N/A'}</Text>
                </View>
