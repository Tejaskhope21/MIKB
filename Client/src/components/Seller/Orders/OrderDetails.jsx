        ...
        <div className="p-6 max-w-7xl mx-auto bg-gray-50 min-h-screen">
                ...
                <div className="bg-white rounded-2xl shadow-lg p-8 mb-8">
                    ...
                    <div className="grid md:grid-cols-2 gap-8">
                        ...
                        <div className="space-y-6">
                            ...
                            <div>
                                ...
                                <div className="bg-gray-50 p-4 rounded-lg space-y-3">
                                    <div className="flex justify-between">
                                        ...
                                        <span className="font-bold text-blue-700">{order.paymentProof.userBankDetails?.bankName || 'Not Provided'}</span>
                                    </div>
                                    {order.paymentProof.userBankDetails?.accountName && (
                                        <div className="flex justify-between">
                                            <span className="text-gray-600">Account Name:</span>
                                            <span className="font-bold">{order.paymentProof.userBankDetails.accountName}</span>
                                        </div>
                                    )}
                                    {order.paymentProof.userBankDetails?.accountNumber && (
                                        <div className="flex justify-between">
                                            <span className="text-gray-600">Account Number:</span>
                                            <span className="font-bold">{order.paymentProof.userBankDetails.accountNumber}</span>
                                        </div>
                                    )}
                                    {order.paymentProof.userBankDetails?.ifscCode && (
                                        <div className="flex justify-between">
                                            <span className="text-gray-600">IFSC Code:</span>
                                            <span className="font-bold">{order.paymentProof.userBankDetails.ifscCode}</span>
                                        </div>
                                    )}
                                    {order.paymentProof.userBankDetails?.upiId && (
                                        <div className="flex justify-between">
                                            <span className="text-gray-600">UPI ID:</span>
                                            <span className="font-bold">{order.paymentProof.userBankDetails.upiId}</span>
                                        </div>
                                    )}
                                    <div className="flex justify-between">
                                        <span className="text-gray-600">Transaction Time:</span>
                                        <span className="font-bold">{order.paymentProof.userBankDetails?.transactionTime || formatDate(order.paymentProof.uploadedAt)}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="space-y-3">
                                    <div className="flex justify-between">
                                        ...
                                        <span className="font-bold text-green-700">{order.paymentProof.transactionId}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-gray-600">Amount Transferred:</span>
                                        <span className="font-bold text-green-700">₹{order.paymentProof.amount?.toLocaleString('en-IN') || order.totalPrice.toLocaleString('en-IN')}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-gray-600">Uploaded by Customer:</span>
                                        <span className="font-bold">{formatDate(order.paymentProof.uploadedAt)}</span>
                                    </div>
                                    {order.paymentProof.verifiedAt && (
                                        <div className="flex justify-between">
                                            <span className="text-gray-600">Verified by You:</span>
                                            <span className="font-bold">{formatDate(order.paymentProof.verifiedAt)}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div>
                                <h3 className="text-lg font-bold text-gray-800 mb-3">Company Account (Where payment should come)</h3>
                                <div className="bg-blue-50 p-4 rounded-lg space-y-2">
                                    <div className="flex justify-between">
                                        <span className="text-gray-600">Account Name:</span>
                                        <span className="font-bold text-blue-700">{order.paymentProof.companyBankDetails?.accountName || "Tejas Khope"}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-gray-600">Account Number:</span>
                                        <span className="font-bold text-blue-700">{order.paymentProof.companyBankDetails?.accountNumber || "970318210000861"}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-gray-600">IFSC Code:</span>
                                        <span className="font-bold text-blue-700">{order.paymentProof.companyBankDetails?.ifscCode || "BKID0009703"}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-gray-600">UPI ID:</span>
                                        <span className="font-bold text-blue-700">{order.paymentProof.companyBankDetails?.upiId || "khopetejas6-1@oksbi"}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-gray-600">Bank:</span>
                                        <span className="font-bold text-blue-700">{order.paymentProof.companyBankDetails?.bankName || "Bank of India"}</span>
                                    </div>
                                </div>
                            </div>

                            {/* Verification Actions */}
                            {!order.paymentProof.verified && order.status === 'payment_pending' && (
                                <div className="mt-6 space-y-4">
                                    <h4 className="font-bold text-gray-800">Verify Payment</h4>
                                    <div className="bg-yellow-50 p-4 rounded-lg mb-4">
                                        <p className="text-sm text-yellow-700 font-medium">
                                            <AlertCircle className="w-4 h-4 inline mr-2" />
                                            Check if payment screenshot matches the details above before verifying.
                                        </p>
                                    </div>
                                    <textarea
                            ...
                            {order.paymentProof.verified !== undefined && (
                                <div className={`p-4 rounded-lg ${order.paymentProof.verified ? 'bg-green-50' : 'bg-red-50'}`}>
                                    <div className="flex items-center gap-2 mb-2">
                                        {order.paymentProof.verified ? (
                                            <>
                                                <Check className="w-5 h-5 text-green-600" />
                                                <span className="font-bold text-green-700">Payment Verified</span>
                                            </>
                                        ) : (
                                            <>
                                    ...
                                    {order.paymentProof.verificationNotes && (
                                        <p className="text-sm text-gray-600">
                                            <span className="font-medium">Notes:</span> {order.paymentProof.verificationNotes}
                                        </p>
                                    )}
                                    {order.paymentProof.rejectionReason && (
                                        <p className="text-sm text-red-600 mt-1">
                                            <span className="font-medium">Reason:</span> {order.paymentProof.rejectionReason}
                                        </p>
                                    )}
                                </div>
                            )}
                        ...
                        <div>
                            <h3 className="text-lg font-bold text-gray-800 mb-4">Payment Screenshot (Uploaded by Customer)</h3>
                            <div className="border-2 border-dashed border-gray-300 rounded-xl p-4">
                                <div className="relative">
                                    <img
                                        ...
                                        onError={(e) => {
                                            e.target.src = 'https://via.placeholder.com/400x500/e0e0e0/666666?text=Payment+Screenshot';
                                ...
                                <div className="mt-6 bg-gray-50 p-4 rounded-lg">
                                    <h4 className="font-bold text-gray-700 mb-2">Verification Checklist:</h4>
                                    <ul className="space-y-2 text-sm text-gray-600">
                                        <li className="flex items-start gap-2">
                                            <div className="w-4 h-4 rounded-full border border-gray-300 flex items-center justify-center mt-0.5">
                                                {order.paymentProof.verified && <Check className="w-3 h-3 text-green-600" />}
                                            </div>
                                            <span>Amount matches: ₹{order.totalPrice.toLocaleString('en-IN')}</span>
                                        </li>
                                        <li className="flex items-start gap-2">
                                            <div className="w-4 h-4 rounded-full border border-gray-300 flex items-center justify-center mt-0.5">
                                                {order.paymentProof.verified && <Check className="w-3 h-3 text-green-600" />}
                                ...
                                <div className="mt-4 flex justify-between items-center">
                                    ...
                                    <button
                                        onClick={() => window.open(order.paymentProof.screenshot, '_blank')}
            ...
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden mb-8" ref={invoiceRef}>
                ...
                <div className="p-10 grid grid-cols-1 lg:grid-cols-3 gap-8 border-b-2 border-gray-200">
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            ...
                            {order.paymentMethod === 'BANK_TRANSFER' && order.paymentProof && (
                                <p className="text-gray-700">
                                    <span className="font-semibold">Transaction ID:</span> {order.paymentProof.transactionId}
                                </p>
                            )}
                        </div>
                    </div>
                    ...
                    <div>
                        <h3 className="text-lg font-semibold text-gray-500 mb-3">SELLER INFO</h3>
                        <div className="space-y-2">
                            <p className="text-gray-700 font-bold">{order.items[0]?.seller?.businessName}</p>
                            <p className="text-gray-700">Phone: {order.items[0]?.seller?.phone || 'N/A'}</p>
                            <p className="text-gray-700">Email: {order.items[0]?.seller?.email || 'N/A'}</p>
                            <p className="text-gray-700">Address: {order.items[0]?.seller?.address || 'N/A'}</p>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="w-full md:w-1/2 lg:w-1/3">
                            <div className="space-y-4">
                                <div className="flex justify-between text-gray-700">
                                    <span>Subtotal:</span>
                                    <span>₹{order.sellerTotal?.toLocaleString('en-IN') || order.totalPrice.toLocaleString('en-IN')}</span>
                                </div>
                                <div className="flex justify-between text-gray-700">
                                    <span>Shipping:</span>
                                    <span>₹0.00</span>
                                ...
                                <div className="flex justify-between text-gray-700">
                                    ...
                                    <span>₹{order.sellerGst?.toLocaleString('en-IN') || (order.totalPrice * 0.18).toLocaleString('en-IN')}</span>
                                </div>
                                <div className="border-t-2 border-gray-300 pt-4">
                                    <div className="flex justify-between text-2xl font-bold">
                                        <span>TOTAL AMOUNT</span>
                                        <span className="text-[#69431f]">₹{order.sellerGrandTotal?.toLocaleString('en-IN') || (order.totalPrice * 1.18).toLocaleString('en-IN')}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div className="bg-[#69431f] text-white p-8">
                    ...
                    <div className="mt-8 pt-6 border-t border-white/20 text-center">
                        ...
                        <p className="text-xs mt-2 opacity-80">©️ {new Date().getFullYear()} BricksIT. All rights reserved.</p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-lg p-8 mb-8">
                ...
                <div className="flex flex-wrap gap-5">
                     ...
                     !order.paymentProof.verified && (
                        <div className="w-full mb-6 p-6 bg-yellow-50 border border-yellow-200 rounded-xl">
                            ...
                            <p className="text-yellow-700">
                                Please verify the payment proof above before processing this order.
                            </p>
                        </div>
                    )}
                    ...
                    {(order.status === 'paid' || 
                      (order.status === 'payment_pending' && order.paymentProof?.verified)) && (
                        <button
