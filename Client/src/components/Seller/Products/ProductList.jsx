...
const ProductList = () => {
  ...
  const [categoryMap, setCategoryMap] = useState({}); // Map category IDs to names
  ...
  useEffect(() => {
    ...
    categories.forEach((cat) => {
      if (cat._id) {
        map[cat._id] =
          cat.name || cat.title || cat.categoryName || "Unnamed Category";
      }
    });
    setCategoryMap(map);
  }, [categories]);
  ...
  const fetchProducts = async () => {
    try {
      ...
      console.log(
        ...
        `${API_URL}/products/my?${queryParams}`,
      );
  ...
  const fetchCategories = async () => {
    try {
      ...
      if (response.data.success) {
        categoriesData = response.data.data || response.data.categories || [];
      ...
      // If no categories from seller endpoint, try public endpoint
      if (categoriesData.length === 0) {
        try {
          console.log("Trying public categories endpoint...");
          const publicResponse = await axios.get(
            `${API_URL}/categories/public/categories`,
            authHeader,
          );
          console.log("Public categories response:", publicResponse.data);

          if (publicResponse.data.success) {
            setCategories(
              publicResponse.data.data || publicResponse.data.categories || [],
            );
          }
        } catch (publicError) {
          console.error("Public categories also failed:", publicError);
        }
      }
    } catch (error) {
      ...
      // Try public endpoint if seller endpoint fails
      try {
        const publicResponse = await axios.get(
          `${API_URL}/categories/public/categories`,
          authHeader,
        );
        console.log("Public categories fallback:", publicResponse.data);
        ...
        // Last resort: try basic categories endpoint
        try {
          const basicResponse = await axios.get(
            `${API_URL}/categories`,
            authHeader,
          );
          console.log("Basic categories fallback:", basicResponse.data);

          if (basicResponse.data.success || basicResponse.data.categories) {
            setCategories(
              basicResponse.data.data || basicResponse.data.categories || [],
            );
          }
        } catch (basicError) {
          console.error("All category endpoints failed:", basicError);
        }
      }
    }
  };
  ...
  const getCategoryName = (product) => {
    ...
    // If category is a string ID, look it up in our map
    if (product.categoryId && typeof product.categoryId === "string") {
      return categoryMap[product.categoryId] || "Uncategorized";
    }

    // If there's a direct category field
    if (product.category && typeof product.category === "object") {
      return product.category.name || product.category.title || "Uncategorized";
    }

    // If product has categoryName directly
    ...
    try {
      ...
      if (action === "delete") {
        ...
        const deletePromises = selectedProducts.map((productId) =>
          axios.delete(`${API_URL}/products/${productId}`, authHeader),
        );
        ...
        const updatePromises = selectedProducts.map((productId) =>
          axios.put(`${API_URL}/products/${productId}`, { status }, authHeader),
        );
  ...
  const exportProducts = async () => {
    try {
      ...
      // Since there might not be an export endpoint, we'll create a JSON export
      const allProductsResponse = await axios.get(
        `${API_URL}/products/my?limit=1000`,
        authHeader,
      );
      ...
      if (allProductsResponse.data.success) {
        ...
        alert(`Exported ${productsData.length} products successfully`);
      } else {
  ...
  // In ProductList.js, update the handleViewProduct function:
  ...
  const getStockStatus = (stock, lowThreshold = 10) => {
    stock = stock || 0;
            ...
            {categories.map((cat) => (
              <option key={cat._id || cat.id} value={cat._id || cat.id}>
                {cat.name ||
                  cat.title ||
                  cat.categoryName ||
                  `Category ${cat._id?.substring(0, 8)}`}
              </option>
            ))}
          </select>
          <button
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      if (e.target.checked && products.length > 0) {
                        setSelectedProducts(
                          products.map((p) => p._id).filter((id) => id),
                        );
                      } else {
                        setSelectedProducts([]);
                      }
                ...
                products.map((product) => {
                  ...
                  return (
                    ...
                    >
                        ...
                        <div className="flex items-center">
                          <div className="h-12 w-12 flex-shrink-0">
                            <img
                              ...
                              src={
                                product.images?.[0] ||
                                `https://via.placeholder.com/48x48?text=${encodeURIComponent(product.name?.charAt(0) || "P")}`
                              }
                              alt={product.name}
                              onError={(e) => {
                                e.target.src = `https://via.placeholder.com/48x48?text=${encodeURIComponent(product.name?.charAt(0) || "P")}`;
                              }}
                            />
                          </div>
                          <div className="ml-4">
                            ...
                            {product.brand && product.brand !== "NAN" && (
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium capitalize ${getStatusColor(product.status)}`}
                        >
                          {product.status
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
          ...
          <div className="bg-white rounded-xl shadow p-4">
            ...
            <div className="text-2xl font-bold text-blue-600">
              ...
              {products
                .reduce(
                  (sum, p) => sum + (p.price || 0) * (p.inventory?.stock || 0),
                  0,
                )
