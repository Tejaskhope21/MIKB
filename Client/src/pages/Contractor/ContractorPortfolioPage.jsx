...
const ContractorPortfolioPage = () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setProjects((prev) =>
          prev.map((p) =>
            p._id === projectId
              ? { ...p, ...response.data.project, updatedAt: new Date() }
              : p,
          ),
        );
      } else {
  ...
  const filteredProjects = projects.filter((project) => {
    const matchesSearch =
      ...
      (project.client?.toLowerCase() || "").includes(
        searchTerm.toLowerCase(),
      ) ||
      (project.location?.toLowerCase() || "").includes(
        searchTerm.toLowerCase(),
      );
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                ...
                <div className="md:col-span-2">
                  ...
                  <p className="text-xs text-gray-500 mt-1">
                    Enter one image URL per line. These images will be displayed
                    in your project gallery.
                  </p>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span
                        className={`px-3 py-1 text-xs font-medium rounded-full ${statusColors[selectedProject.status] || "bg-gray-100 text-gray-800"}`}
                      >
                        {selectedProject.status?.charAt(0).toUpperCase() +
                          selectedProject.status?.slice(1) || "Unknown"}
                      </span>
                    </div>
                  </div>
                </div>
