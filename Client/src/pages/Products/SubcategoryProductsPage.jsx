...
const SubcategoryProductsPage = () => {
  ...
  const fetchSubcategoryProducts = async () => {
    try {
      ...
      const brands = [
        ...new Set(result.products.items.map((p) => p.brand).filter(Boolean)),
      ];
  ...
  const fetchCategoryProductsWithFilter = async () => {
    try {
      ...
      if (filters.materialType !== "all") {
        finalProducts = finalProducts.filter(
          (product) => product.materialType === filters.materialType,
        );
      }
      ...
      if (categoryResponse.ok) {
        ...
        if (categoryData.success) {
          ...
          if (categoryData.category?.subcategories) {
            const subcat = categoryData.category.subcategories.find(
              (s) =>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-gray-900 mb-2">
                {subcategory?.title || "Subcategory Products"}
                {category && (
                  <span className="text-gray-600"> in {category.name}</span>
                )}
              </h1>
          ...
          <div className="lg:col-span-1 space-y-6">
            ...
            <div className="bg-white rounded-lg border p-5 shadow-sm">
              ...
              <div className="space-y-3">
                ...
                {productStats.minPrice < Infinity &&
