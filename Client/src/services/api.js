...
const safeFetch = async (url, options = {}) => {
  try {
    ...
    if (error.message.includes("Failed to fetch")) {
      throw new Error(
        `Cannot connect to server at ${API_BASE_URL}. Is the backend running?`,
      );
    }
    throw error;
  }
...
export const fetchCategories = async () => {
  try {
    const data = await safeFetch(`${API_BASE_URL}/categories/public/`);
...
export const fetchProductsByCategory = async (categoryId) => {
  try {
    const data = await safeFetch(
      `${API_BASE_URL}/products/category/${categoryId}?showAll=true`,
    );
    ...
    if (data.success === false && data.message?.includes("Route not found")) {
      ...
      return (
        subcategories.find(
          (s) =>
            s._id === subcategoryId ||
            s.numericId?.toString() === subcategoryId,
...
const sortProducts = (products, sortBy) => {
  return [...products].sort((a, b) => {
    switch (sortBy) {
      ...
      case "discount":
        return (
          (b.discount || b.discountPercentage || 0) -
          (a.discount || a.discountPercentage || 0)
        );
      case "name-asc":
...
const fallbackProductsBySubcategory = async (
  categoryId,
  subcategoryId,
  filters = {},
) => {
  try {
    ...
    if (filters.minPrice)
      filtered = filtered.filter(
        (p) => (p.price || 0) >= parseFloat(filters.minPrice),
      );
    if (filters.maxPrice)
      filtered = filtered.filter(
        (p) => (p.price || 0) <= parseFloat(filters.maxPrice),
      );
    if (filters.brand)
...
export const fetchProductsBySubcategory = async (
  ...
  try {
    ...
    if (data.success === false) {
      return await fallbackProductsBySubcategory(
        categoryId,
        subcategoryId,
        filters,
      );
...
export const formatRequirementForDisplay = (requirement) => ({
  ...
  totalQuantity:
    requirement.materials?.reduce((sum, m) => sum + (m.quantity || 0), 0) || 0,
