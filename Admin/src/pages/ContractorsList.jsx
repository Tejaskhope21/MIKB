...
const ContractorsList = () => {
    ...
    const fetchContractors = async () => {
        try {
            ...
            if (filter === 'verified') {
                filteredData = filteredData.filter(c => c.isVerified && c.verificationStatus === 'verified');
            } else if (filter === 'pending') {
                filteredData = filteredData.filter(c => c.verificationStatus === 'pending');
            } else if (filter === 'rejected') {
                filteredData = filteredData.filter(c => c.verificationStatus === 'rejected');
            }
    ...
    const handleVerify = async (contractorId) => {
        ...
        try {
            ...
            await axios.put(`${API_URL}/admin/contractors/${contractorId}/verify`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleUnverify = async (contractorId) => {
        if (!window.confirm('Are you sure you want to reject/unverify this contractor?')) return;

        try {
            const token = localStorage.getItem('token');
            
            await axios.put(`${API_URL}/admin/contractors/${contractorId}/unverify`, {}, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            await axios.delete(`${API_URL}/admin/contractors/${contractorToDelete._id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    const handleBulkVerify = async () => {
        ...
        if (!window.confirm(`Verify ${selectedContractors.length} selected contractors?`)) return;
    ...
    const exportToCSV = () => {
        const headers = ['Name', 'Company', 'Email', 'Phone', 'Type', 'Experience', 'Status', 'City', 'State', 'Join Date'];
                ...
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-4">
                        ...
                        <span className="text-gray-600">
                            {selectedContractors.length} of {filteredContractors.length} selected
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-lg overflow-hidden">
                {loading ? (
                    ...
                    <div className="p-12 text-center">
                        ...
                        <h3 className="text-xl font-bold text-gray-700 mb-2">No Contractors Found</h3>
                        <p className="text-gray-500">
                            {search ? 'Try different search terms' : 'No contractors available yet'}
                        </p>
                    </div>
                ) : (
                        ...
                        <table className="w-full">
                            <thead className="bg-gray-50">
                                <tr>
                                    ...
                                    <th className="py-4 px-6 text-left font-semibold text-gray-700">Contractor</th>
                                    ...
                                    <tr key={contractor._id} className="hover:bg-gray-50">
                                        ...
                                        <td className="py-4 px-6">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <p className="font-bold text-gray-900">{contractor.companyName}</p>
                                                    <p className="text-sm text-gray-600">{contractor.name}</p>
                                                    <p className="text-xs text-gray-500">{contractor.contractorType}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="py-4 px-6">
                                        ...
                                        <td className="py-4 px-6">
                                            <div className="space-y-2">
                                                <p className="text-sm">
                                                    <span className="font-medium">Experience:</span> {contractor.experience} years
                                                </p>
                                                <p className="text-sm">
                                                    <span className="font-medium">Team:</span> {contractor.teamSize}
                                                </p>
                                                <p className="flex items-center gap-2 text-sm">
                                                    <MapPin className="w-4 h-4 text-gray-400" />
                                                    {contractor.address?.city || 'N/A'}, {contractor.address?.state || 'N/A'}
                                                </p>
                                            </div>
                                        </td>
                                        <td className="py-4 px-6">
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                    <div className="bg-white rounded-2xl p-8 max-w-md w-full">
                        <div className="text-center mb-6">
                            ...
                            <h3 className="text-2xl font-bold text-gray-900 mb-2">Delete Contractor</h3>
                            <p className="text-gray-600">
                                Are you sure you want to delete <strong>{contractorToDelete.companyName}</strong>?
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-red-600 mt-1">
                                        This will permanently delete the contractor's account, portfolio, and all associated data.
                                    </p>
                                </div>
                            </div>
                        </div>
